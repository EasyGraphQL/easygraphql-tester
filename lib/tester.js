...
class Tester {
  ...
  test (testName, expectedValue, query, args) {
    if (!testName) {
      throw new Error('Test name is missing')
    }

    if (typeof expectedValue !== 'boolean') {
      throw new Error('Should Throw Error result must be a boolean')
    }
    
    if (expectedValue) {
      this.mock(query, args)
      it(testName, () => {
        return assert.strictEqual(expectedValue, true, testName);
      })
    } else {
      let error
      try {
      ...
      if (!error) {
        throw new Error(`${testName} failed, there should be an error and it pass`)
      }
      
      it(testName, () => {
        return assert.strictEqual(expectedValue, false, testName);
      })
    }
  }
}
